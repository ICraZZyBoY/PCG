...
void MainWindow::processSegments()
{
    ...
    plot->addLine( {QPointF(winX1, winY1), QPointF(winX2, winY1), Qt::blue} );
    plot->addLine( {QPointF(winX1, winY1), QPointF(winX1, winY2), Qt::blue} );
    plot->addLine( {QPointF(winX2, winY2), QPointF(winX1, winY2), Qt::blue} );
    plot->addLine( {QPointF(winX2, winY2), QPointF(winX2, winY1), Qt::blue} );
    for (int i = 0; i < n; ++i)
    {
        ...
        while (firstCode | secondCode)
        {
            ...
            {
                plot->addLine( {first, second, Qt::red} );
                break;
            }
            if (firstCode)
            {
            ...
            if (code == firstCode)
            {
                plot->addLine( {first, cur, Qt::red} );
                first = cur;
                firstCode = getCode(winLeft, winRight, first);
            }
            else
            {
                plot->addLine( {second, cur, Qt::red} );
                second = cur;
                secondCode = getCode(winLeft, winRight, second);
            }
        }
        if (!(firstCode & secondCode))
            plot->addLine( {first, second, Qt::green } );
    }
    fin.close();
}
...
QPointF intersect(QPointF first, QPointF second, QPointF third, QPointF fourth)
{
    ...
    return QPointF(double(numX) / den, double(numY) / den);
}
...
void MainWindow::processPolygons()
{
    ...
    {
        plot->addLine({points[i], points[(i + 1) % points.size()], Qt::red});
    }
    for (int i = 0; i < visible.size(); ++i)
    {
        plot->addLine({visible[i], visible[(i + 1) % visible.size()], Qt::green});
    }
    fin.close();
}
